...
def main() -> None:
    ...
    raw = pd.read_csv(ROOT / "Student_data.csv")
    assert list(raw.columns) == PROJECT_COLUMNS, list(raw.columns)
    source_hash = sha256((ROOT / "Student_data.csv").read_bytes()).hexdigest()
    assert source_hash == "bfadc93d30f7341ed83f5eb1cd793d4cc69e47a4fb532522469929cafd77e9c6"
    ...
    print(f"  OK: processed four-feature project CSV, {len(frame):,} valid records and all target classes")

    print("3/6 Checking feature evidence...")
    relevance = feature_relevance(frame)
    included = set(relevance.loc[relevance["Decision"] == "Included", "Feature Key"])
    assert included == set(FEATURES), included
    correlations = relevance.set_index("Feature Key")["Pearson Correlation"]
    assert correlations["Average_Score"] > 0.80
    assert correlations["Previous_CGPA"] > 0.85
    assert correlations["Attendance_Pct"] > 0.25
    assert correlations["Study_Hours_Per_Day"] > 0.20
    for excluded in ["Number_of_Subjects", "Age", "Sleep_Hours", "Social_Hours_Week"]:
        assert abs(correlations[excluded]) < 0.05, (excluded, correlations[excluded])
    print("  OK: all four selected features are related; unrelated variables are excluded")
    ...
    batch_columns = ["Number_of_Subjects"] + FEATURES
    load_batch = pd.concat([frame[batch_columns], frame[batch_columns]], ignore_index=True)
    validated_load, load_errors = validate_batch(load_batch)
    assert not load_errors, load_errors
    load_result = predict_batch(training.models, validated_load, training.best_model)
    assert len(load_result) == 10_000
    assert load_result["Final_Confidence"].between(0.0, 1.0).all()
    print(
        f"  OK: individual={prediction}, batch={batch_result.loc[0, 'Final_Prediction']}, "
        f"load={len(load_result):,} records"
    )
