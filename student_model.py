...
PROFILE_FIELDS = ["Number_of_Subjects"]
...
PROJECT_COLUMNS = KAGGLE_COLUMNS + ["Number_of_Subjects", "Average_Score"]
...
FEATURE_LABELS = {
    ...
    "Social_Hours_Week": "Social Hours per Week",
    "Age": "Age",
}
...
def prepare_dataset(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = raw[PROJECT_COLUMNS].copy()
    numeric_columns = CANDIDATE_NUMERIC_FEATURES + ["Final_CGPA"]
    for column in numeric_columns:
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=KAGGLE_COLUMNS).copy()

    frame = frame[frame["Age"].between(18, 24, inclusive="both")]
    frame = frame[frame["Attendance_Pct"].between(0.0, 100.0, inclusive="both")]
    frame = frame[frame["Study_Hours_Per_Day"].between(0.0, 24.0, inclusive="both")]
    frame = frame[frame["Previous_CGPA"].between(0.0, 4.0, inclusive="both")]
    frame = frame[frame["Sleep_Hours"].between(0.0, 24.0, inclusive="both")]
    frame = frame[frame["Social_Hours_Week"].between(0.0, 168.0, inclusive="both")]
    frame = frame[frame["Final_CGPA"].between(0.0, 4.0, inclusive="both")]
    frame = frame[frame["Number_of_Subjects"].between(1, 12, inclusive="both")]
    frame = frame[frame["Average_Score"].between(0.0, 100.0, inclusive="both")]
...
def validate_batch(raw: pd.DataFrame, maximum_rows: int = 10_000) -> tuple[pd.DataFrame, list[str]]:
    ...
    errors: list[str] = []
    missing = [feature for feature in PROFILE_FIELDS + FEATURES if feature not in raw.columns]
    if missing:
    ...
    cleaned = raw.copy()
    subject_count = pd.to_numeric(cleaned["Number_of_Subjects"], errors="coerce")
    invalid_subjects = subject_count.isna() | ~subject_count.between(1, 12) | (subject_count % 1 != 0)
    if invalid_subjects.any():
        errors.append(
            "Number of Subjects has "
            f"{int(invalid_subjects.sum())} invalid value(s); use whole numbers from 1 to 12."
        )
    cleaned["Number_of_Subjects"] = subject_count
...
def feature_relevance(frame: pd.DataFrame) -> pd.DataFrame:
    """Audit every numeric Kaggle candidate against the continuous target."""
    information = mutual_info_regression(
        frame[CANDIDATE_NUMERIC_FEATURES], frame["Final_CGPA"], random_state=RANDOM_STATE
    )
    pearson = frame[CANDIDATE_NUMERIC_FEATURES].apply(
        lambda series: series.corr(frame["Final_CGPA"], method="pearson")
    )
    spearman = frame[CANDIDATE_NUMERIC_FEATURES].apply(
        lambda series: series.corr(frame["Final_CGPA"], method="spearman")
    )
    result = pd.DataFrame(
        {
            "Feature Key": CANDIDATE_NUMERIC_FEATURES,
            "Feature": [FEATURE_LABELS[feature] for feature in CANDIDATE_NUMERIC_FEATURES],
            "Pearson Correlation": [pearson[feature] for feature in CANDIDATE_NUMERIC_FEATURES],
            "Spearman Correlation": [spearman[feature] for feature in CANDIDATE_NUMERIC_FEATURES],
            "Mutual Information": information,
            "Decision": ["Included" if feature in FEATURES else "Excluded" for feature in CANDIDATE_NUMERIC_FEATURES],
        }
    )
    result["_Selected Rank"] = result["Decision"].map({"Included": 0, "Excluded": 1})
    result["_Association Strength"] = result["Pearson Correlation"].abs()
    return (
        result.sort_values(["_Selected Rank", "_Association Strength"], ascending=[True, False])
        .drop(columns=["_Selected Rank", "_Association Strength"])
        .reset_index(drop=True)
    )
